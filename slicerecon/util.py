...
def prescale_and_filter_all(original_vectors, proj):
    for i in tqdm.tqdm(np.arange(0, proj.shape[1]), desc='filter proj'):
        proj[:, i, :] = prescale_and_filter(original_vectors, proj[:, i, :], i)
